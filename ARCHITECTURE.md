...
- **S9 — RikuOS will not hold a second copy of the Meta page token. The Graph API ping becomes a ShikksTracker contract change.** §5 and the roadmap's task 5.1 both specified a direct Graph API ping as one of the watchdog's checks, deferred while ShikksTracker's P2 was unbuilt (design P5a-1). P2 shipped and the deferred half was picked up on 2026-09-04; the ping did not survive contact with the facts.
  ...
  - **What ships instead, and what it does not cover.** Webhook liveness: `summary.messenger.lastEventAt` advances on any Meta delivery, inbound or echoed, so an expired token freezes it. RikuOS alarms after **10 days** of total silence — Riku's threshold, chosen over 7 and 14 on 2026-09-04, calibrated against volume rather than uptime per ShikksTracker's own contract note that the page sees "a handful of messages a week". **It gives no advance warning of expiry** — the alarm fires up to 10 days *after* Meta stops delivering, so detection trails the failure rather than preceding it. What 10 days buys is that the trailing gap is a sixth of the token's ~60-day life, so an expiry is caught and regenerated inside the same cycle instead of surfacing when a lead complains they were ignored. It also cannot distinguish "token expired" from "nobody messaged the page". A direct token check is still worth having; it belongs in the repo that holds the token. Proposed as a contract addition (`messenger.tokenOk`, `messenger.tokenCheckedAt`) in `docs/handoffs/2026-09-04-meta-token-health.md`.
...
- **A running send engine is not a sending send engine, and the monitor cannot tell them apart.** Found 2026-09-04 while verifying that ShikksTracker's hourly pinger had been wired up (it has: `.github/workflows/sequence-pinger.yml`, commit `61d36bb`, and `engine.lastRunAt` now reads same-day after 29 days frozen). That commit's own message records that both kill switches — `draftGenerationEnabled` and `sendingEnabled` — are `false` in the live Settings document, so the pinger restarted reply detection and nothing else. The outreach monitor reads a fresh `lastRunAt` as healthy, so the morning digest would report all clear while no outreach leaves the building at all. RikuOS cannot close this from here: `GET /api/os/summary` exposes no toggle state, so judging it is another OS-API contract addition (S4). Deliberately left open — if the switches are meant to be off, the silence is correct, and only Riku knows which it is.
