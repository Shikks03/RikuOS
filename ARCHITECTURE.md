...
- **S9 — RikuOS will not hold a second copy of the Meta page token. The Graph API ping becomes a ShikksTracker contract change.** §5 and the roadmap's task 5.1 both specified a direct Graph API ping as one of the watchdog's checks, deferred while ShikksTracker's P2 was unbuilt (design P5a-1). P2 shipped and the deferred half was picked up on 2026-09-04; the ping did not survive contact with the facts.
  ...
  - **What shipped instead — and it is gone too, as of S15 (2026-09-05).** A webhook-liveness check stood in for the token ping: `summary.messenger.lastEventAt` advanced on any Meta delivery, so an expired token froze it, and RikuOS alarmed after **10 days** of silence (Riku's threshold, chosen over 7 and 14 on 2026-09-04). It never gave advance warning — detection trailed the failure by up to eleven days — and it could not tell "token expired" from "nobody messaged the page". **S15 deleted it along with the rest of the Messenger lane**, because ShikksTracker stopped sending the `messenger` block and a missing block reads as `null`, which was the "never fired" branch: keeping it would have alarmed every morning about a system that no longer exists. The direct token check proposed as a contract addition (`messenger.tokenOk`, `messenger.tokenCheckedAt`) is **withdrawn, not pending** — there is no subscription left whose token expiry could matter — and the handoff carrying it is deleted. **This entry is history, not a live design.** Nothing here is to be rebuilt without a new decision.
...
- **S15 — The Facebook Messenger lane is dropped entirely, in both repos. P6 triage is deleted, not parked.** Riku's call, 2026-09-05. Advanced Access for `pages_messaging` requires Meta's App Review, and App Review requires business verification. "Riku" is a deliberate side thing with **no registered legal entity and no plan to register with DTI or SEC**, so publishing the app is permanently blocked rather than pending. Registration as a workaround is explicitly ruled out and is a standing constraint — do not re-propose it.
  ...
  - **The cut is deliberate and total, not a pause.** P6 shipped complete on 2026-09-05 (457 tests, deployed, live-verified) and was deleted the same day anyway, because its only input is inbound prospect DMs. Keeping working code with no possible input is worse than removing it: it invites a future session to "finish" or revive it, and it leaves live monitoring wired to a signal that can never move. **Done in both repos on 2026-09-05** — RikuOS `356197f` (457 tests → 300, deployed and verified live), ShikksTracker `cb3d2d7` and `2b84d9a`, with both repos' leftover database objects cleared. The handoff `docs/handoffs/2026-09-04-p6-messenger-forwarding.md` was withdrawn and deleted — the two ShikksTracker contracts it requested were never built and are not to be.
