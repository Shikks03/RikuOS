...
| D6 | **An explicit "never claim / always defer" list bounds the draft — held in the prompt code, not in the editable block.** (Revised during planning: the rules were first placed in the block itself. A safety rule living in a settings field is one Riku can delete by accident while trimming the block to fit its 4,000-character cap. The block holds facts; the code holds the rules about them.) | Riku had zero paid clients as of August 2026 and the vault records no portfolio URLs or contact details. An LLM given silence improvises, and a draft inventing client work is the worst realistic failure of this feature. Bounding it structurally beats hoping the model is careful. |
